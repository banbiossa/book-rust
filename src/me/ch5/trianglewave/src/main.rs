...
fn main() {
    ...
    let mut fw = hound::WavWriter::create("tri_4.wav", spec).unwrap();
...
fn calc_len(bpm: usize, n: usize) -> usize {
    ((4.0 / n as f32) * (60.0 / bpm as f32) * CHANNELS as f32 * SAMPLE_RATE) as usize
}

fn tri_wave(noteno: i32, len: usize, gain: f32) -> Vec<f32> {
    let tone = noteno_to_hz(noteno);
    let form_samples = SAMPLE_RATE * CHANNELS as f32 / tone;
